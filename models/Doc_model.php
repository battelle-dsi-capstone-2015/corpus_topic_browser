...
#Take the square root of that and we are done!
  ...
  function get_helsinger_distro()
  {
    $q = $this->db->query("SELECT ROUND(distance,2) as 'd', count(*) as 'n' FROM docpair GROUP BY d ORDER BY d");
    return $q->result_array();
  }
  ...
  function get_string($term)
  {
    ...
    $q = $this->db->query("
			SELECT DISTINCT title as 'label', doc_id as 'value'
			FROM src_all_doi
